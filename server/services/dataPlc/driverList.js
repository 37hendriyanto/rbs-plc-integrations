...
const service = {
    ...
    process: async function (input, db) {
        ...
        // INIT VALUE
        input.orderBy = input.orderBy ? input.orderBy : 'A.name'
        ...
        var sql = `SELECT A.* FROM (
                        SELECT A.id_driver AS id, A.nama_supir AS name,
        ...
        var sql2 = `SELECT COUNT(A.id) AS record FROM (
                        SELECT A.id_driver AS id, A.nama_supir AS name, CASE WHEN uuid IS NOT NULL THEN '1' ELSE '0' END AS sync
                        FROM master_driver A
                    ) A ${input.filter}`
