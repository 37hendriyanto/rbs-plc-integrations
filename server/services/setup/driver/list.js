...
const service = {
    ...
    process: async function (input, db) {
        ...
        var sql = `SELECT A.* FROM (
                        SELECT A.uuid AS id, A.name,CASE WHEN B.uuid IS NOT NULL THEN '1' ELSE '0' END AS sync, B.nama_supir AS plc_name,A.active
                        FROM rbs_driver A
                        LEFT JOIN master_driver B ON B.uuid = A.uuid
        ...
        var sql2 = `SELECT COUNT(A.id) AS record
                    FROM (
                        SELECT A.id, A.uuid, A.name,CASE WHEN B.uuid IS NOT NULL THEN '1' ELSE '0' END AS sync, B.nama_supir AS plc_name
                        FROM rbs_driver A
                        LEFT JOIN master_driver B ON B.uuid = A.uuid
                    ) A ${input.filter}`
